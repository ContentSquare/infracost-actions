...
func TestPullRequest_Errors(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantMessage: `repo URL "https://github.com/infracost/actions#frag" must not contain a query or fragment`,
		},
		{
			name:        "query",
			provider:    ProviderGitHub,
			repoURL:     "https://github.com/infracost/actions?x=1",
			wantMessage: `repo URL "https://github.com/infracost/actions?x=1" must not contain a query or fragment`,
		},
		{
