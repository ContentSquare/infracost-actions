...
func TestBuildRunInput_PullRequestURL(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.provider, input.Metadata["vcsProvider"])
...
func TestBuildErrorRunInput_PullRequestURL(t *testing.T) {
	input := BuildErrorRunInput(RunInputOptions{
		VCSProvider: "gitlab",
		RepoURL:     "https://gitlab.com/infracost/actions",
		PRNumber:    7,
	}, 0, "Failed to scan", "boom")

	assert.Equal(t, "error", input.Metadata["command"])
	assert.Equal(t, "gitlab", input.Metadata["vcsProvider"])
	assert.Equal(t, "https://gitlab.com/infracost/actions/-/merge_requests/7", input.Metadata["vcsPullRequestUrl"])
}
